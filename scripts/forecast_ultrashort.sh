...
# 자료구분코드, 예보값,예보일자, 예보시간, 예보지점X좌표,예보지점Y좌표의 정보를 조회하는 기능	


# validate
if [ "$#" -lt "5" ]; then
  echo "Usage: forecast_ultrashort.sh {SERVICE_KEY} {BASE_DATE} {BASE_TIME} {NX} {NY} {PAGE=1} {SIZE=10}" 
  exit 1
fi
...
# config
REQUEST_URL=http://newsky2.kma.go.kr/service/SecndSrtpdFrcstInfoService2/ForecastTimeData
